# Runbook: dbt-core + Snowflake

Step-by-step setup and operational instructions for this project. For a project overview, architecture, and the data model, see the main [README.md](../README.md).
...
(`<project>` is the container name prefix Astro CLI generated for this project — find it with `astro dev ps` or `docker ps`.) `dbt debug` should report `Connection test: [OK connection ok]`; the "git" dependency check failing is expected and harmless unless `dbt_sales_mart/dependencies.yml` starts pulling packages from git.
